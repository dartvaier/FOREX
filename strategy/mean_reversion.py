...
class SimpleMeanReversionStrategy:
    """
    Long-only short-horizon mean reversion baseline.

    Rules:

    - cumulative return over lookback bars is below the
      negative entry threshold -> ENTER_LONG
    - cumulative return recovers to exit threshold or above
      -> EXIT
    - otherwise -> HOLD
    ...
    strategy_id: str = "SIMPLE-MEAN-REVERSION"
    ...
    def __post_init__(self) -> None:
        ...
        if self.entry_threshold >= self.exit_threshold:
            raise ValueError(
                "entry_threshold must be smaller than "
                "exit_threshold"
            )
    ...
    def on_bar(
        ...
        if reversion_return < self.entry_threshold:
            return self._signal(
                context,
                action=SignalAction.ENTER_LONG,
                reason="Negative move reached mean reversion threshold",
                metadata=metadata,
            )

        if reversion_return >= self.exit_threshold:
            return self._signal(
                context,
                action=SignalAction.EXIT,
                reason="Mean reversion recovered to exit threshold",
                metadata=metadata,
            )
